...
- **재검증 결과(2026-07-19, Phase 1~4 전체 재검증 중)**: 위 "재검토 필요" 항목을 `panel.cpp`의 `AddLine`/`FetchSpeaker`/`ReplaceBody`/`AvatarInPanel`을 직접 대조해 재검토 완료 — 실제로는 문제없음을 확인했다. `AddLine`은 새/클론 패널을 만들 때 `oldP->AvatarInPanel(id)`가 참이면 무조건 새 패널을 시작하므로, 현재 줄의 화자 `id`는 `FetchSpeaker(newP)` 시점에 `newP->m_bodies`에 절대 이미 존재할 수 없다 — 즉 `FetchSpeaker`의 "이미 있음" 조기 반환(`RecordBody` 미호출) 분기는 그 줄의 화자에 대해 결코 일어나지 않고, `RecordBody`는 항상(뒤이은 `ReplaceBody` 호출까지 포함하면 멱등하게 두 번) 호출된다. 레이아웃 실패로 `AddLine`이 재귀 재시도되어도 `av->m_body`가 그사이 바뀌지 않으므로 결과는 같다. 따라서 "메시지 하나 = 매칭 즉시 확정"으로 매번 갱신하는 `apps/server/src/room.ts`의 현재 구현은 원작과 항상 동일한 순서를 관찰하며, 별도 수정이 필요 없다(주석도 이 재검증 결과로 갱신함).
